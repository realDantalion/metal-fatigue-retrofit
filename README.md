...
- **You installed a fan translation (e.g. the Portuguese one) and the patcher no longer recognises the game.** The patcher checks `MFatigue.exe` itself. If the translation replaced that file, it is no longer the build the patcher knows, and it refuses on purpose. Restore the original EXE first (on Steam: *Verify integrity of game files*), patch that, and re-apply the translation afterwards. If the translation replaces `MFatigue.exe` rather than just data files, it will overwrite the patch — in that case you can only have one or the other.
